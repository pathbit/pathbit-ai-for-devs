...
21f80dc8f517   ghcr.io/pathbit/9rtksync:latest   router-sync   Up 4 minutes (healthy)
...
| **Token Sync Sidecar** | `router-sync` | `9190` (painel) | 9RTKSync em Python 3.14 Alpine para auto-renovação de tokens e prevenção do bug ISO |
...
- O container sidecar `router-sync` roda silenciosamente consumindo 27,53 MiB de RAM (`docker stats --no-stream router-sync`, medido em 2026-09-13). Ele checa a validade do token a cada 5 minutos e renova automaticamente 15 minutos antes de expirar, eliminando erros 401 e 503.
