...
Além do passo a passo visual com 41 capturas de tela cobrindo desde a obtenção das chaves até a operação no terminal, detalharemos a fundo a arquitetura de configurações do Claude Code, esclarecendo a diferença entre `settings.json` e `settings.local.json`, o que o sufixo `[1m]` realmente faz, como a escolha do menu `/model` vaza para a sua conta pessoal e por que o Advisor experimental deve ser rigorosamente desativado em qualquer integração com terceiros.
